...
import { vehicleLookup } from '@/lib/vehicle-qr';
...
export default function VehicleCheckout({ initialVehicleId }: { initialVehicleId?: string }) {
  ...
  const handleScanQR = async (codeToSearch: string) => {
    ...
    try {
      const query = codeToSearch.startsWith('vehicle:')
        ? { vehicleId: codeToSearch.slice(8) }
        : vehicleLookup(codeToSearch, window.location.origin);
      const response = await fetch('/api/vehicle-usage?' + new URLSearchParams(query), { cache: 'no-store' });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'ไม่พบรถยนต์');
      const targetVehicle = data.vehicle;
      const activeBookings = data.bookings;
      setScannedVehicle(targetVehicle);
      setVehicleBookings(activeBookings);
      setActiveWalkInLog(data.activeWalkInLog);
      ...
      showError('ดึงข้อมูลไม่สำเร็จ', e instanceof Error ? e.message : 'ไม่สามารถติดต่อฐานข้อมูลได้');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (initialVehicleId) void handleScanQR('vehicle:' + initialVehicleId);
    // The route is the source of the initial selection.
    ...
    if (formValues) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          await handleScanQR('vehicle:' + vId);
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 mt-8 relative z-10">
        {initialVehicleId ? (
          <div className="mb-6 text-center">
            <p className="text-slate-600">{loading ? 'กำลังโหลดข้อมูลรถ...' : 'หน้ารับ–คืนรถจาก QR Code'}</p>
            <button disabled={loading} onClick={() => handleScanQR('vehicle:' + initialVehicleId)} className="mt-2 text-purple-700 underline disabled:opacity-50">โหลดข้อมูลรถอีกครั้ง</button>
          </div>
